...
此外后台标签的定时器被节流（实测 `setTimeout(1000)` 实际耗时约 2400ms），滚动循环会明显变慢。

**解法**：新标签开出来后**先前台化一次**（截个图就够）再交给它任务；之后不必反复抢前台。
所以多开**有收益但不是 N 倍**，约 1.5–2 倍——瓶颈是节流，不是并发数。
另外别为了抢前台去打断用户正在用的浏览器，慢一点没关系。
