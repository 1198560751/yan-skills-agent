...
| 7.5 | 品牌资产与测量接入 | **段 4 · 上线前 SEO/GEO**（A 品牌资产、D 七行闸门）+ **段 5 · 上线与接入**（A2 邮箱、B/C 接入拆两批） | 一拆为二：预览域上能做的进段 4，依赖正式域名的进段 5 |
...
| **每次页面改动全套体检重跑**（下 D 表 0–6 + 4b 全部），不是只跑某几行 | 改一处 TDK 可能带坏密度，改一个区块可能带坏 CLS；只重跑某两行等于默认其余没变，而这正是清单腐坏的起点 |
...
**C. 上线前闸门：七行硬性检查（0–6）+ 4b，逐行要证据，不是工具清单**
