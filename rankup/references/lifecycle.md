...
13. **同意门槛会让不同工具差一个数量级，这不是任何一方的 bug。**
    ...
    托管方的无 cookie 分析统计所有人。实测两者可以差**接近一个数量级**。
    **写结论前先问这个数字来自哪一侧**——拿被门槛过滤过的数字判断「没人用」是常见误判。
...
7. **产品发布平台（Product Hunt 一类）。** 它给的是曝光、推荐流量和种子用户。
   三条实测约束：
   - **账号形态是硬门槛，不是产品。** 必须是**看起来像真实用户的个人号**——
     头像、ID、简介齐全，且平时真的用它投票评论。被判定为品牌号会导致发帖与评论被禁用。
   - **只能正经发一次。** 素材（缩略图 + 画廊图）是这类页面最显眼的部分，
     宁可晚一天带齐图再发，也不要用一张凑合的图把这一次用掉。
   - **`rel` 要实测再记账。** 这类平台的产品外链通常是 `nofollow`；
     但 `nofollow` 不等于零——把它当作提示信号与外链盘自然度的一部分，
     而不是当作权重来源或当作没有。

   ⚠️ **一个自动化边界，提前知道能省很多时间**：这类站的图片上传走的是隐藏 file input，
   而**浏览器自动化连接器通常无法为 file input 设置文件**（隐藏或可见都一样被拒）。
   所以「填表全自动、图片人工拖」是这条渠道的正常形态，不是失败。
   把文案、标签、首条评论全部填好并存成草稿，再把图交给用户拖，是正确的分工。
