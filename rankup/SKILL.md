...
## 从一句话反查能力（用户不知道有哪些功能，你得知道）

**用户不会说「跑一下 seo-audit.mjs」，他会说「今天想弄下 SEO」。**
这一节负责把模糊人话翻译成具体入口。全量能力底账在
...
| 用户可能说的话 | 先看什么 → 再跑什么 → 判读依据在哪 |
|---|---|
| 「新建个站」「想做个工具站」「帮我搭起来」 | [`lifecycle.md`](references/lifecycle.md) 阶段 2–4 → 本文「默认建站栈」的脚手架命令 + `scripts/cf-zone-setup.mjs`（域名进 CF）→ 闸门判据 [`checklists.md`](references/checklists.md) 阶段 3/4 |
...
**这张表只给入口，不重复判据。** 下方「本 Skill 自带」是脚本明细表，
「任务路由」是更细的参考文档索引；两处都不需要在这里再抄一遍。
...
| `scripts/gt-browser.mjs` | gt 的 OpenCLI 取数层：驱动已登录 Chrome，在 trends.google.com 页面里 fetch Trends 内部 widget 接口 | 一般不直接调，由 `gt.py` 转发。pytrends 的 429 就是靠它绕开的；要它工作先 `opencli doctor`。每次运行落 `trends-<kw>.json`+截图+manifest 进 `.rankup/evidence/gt-browser-<ts>/`；**空曲线不再被说成「太冷门」**——「没取到」与「没需求」不可分辨时脚本如实说不可分辨，判读看证据目录 |
...
| **`scripts/webcafe-forum.mjs`** + `webcafe-transport.mjs` + `webcafe-rsc.mjs` | **new.web.cafe（哥飞论坛）全站取数**：`get <任意站内 URL>` 万能入口；悬赏问答（含 `collect` 征集榜单与提交理由）、经验 91 条 / 帖子 722 条 / 教程专栏 40 个、站内搜索、**14 个微信群归档搜索**、哥飞.ai 历史对话 | 要哥飞社区里的原始素材、经验帖、悬赏答案、群里的原话时。**先读 [`webcafe-forum.md`](references/webcafe-forum.md)**——这个站匿名不会 401，它返回 200 但把正文抹成空串。正文可见性输出为 `access_evidence`（判据原文）+ `suggested_access`（建议非判决）；取数失败 die 前会把状态码/原文片段（浏览器路径再加截图）落 `.rankup/evidence/` |
