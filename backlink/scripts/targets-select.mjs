...
const a = { cohort: [], kind: [], freeOnly: false, paidOk: false, maxAge: null, limit: Infinity, format: 'table', stats: false, unattended: false };
...
if (a.stats) {
  const rows = {};
  for (const t of all) {
    rows[t.cohort] ??= { open: 0, optional: 0, required: 0, unknown: 0, total: 0 };
  ...
  process.stdout.write(`cohort            total  no-cost  free+paid  paid-only  unknown\n`);
  for (const [c, r] of Object.entries(rows).sort((x, y) => y[1].total - x[1].total)) {
    process.stdout.write(`${c.padEnd(17)}${String(r.total).padStart(5)}${String(r.open).padStart(9)}${String(r.optional).padStart(11)}${String(r.required).padStart(11)}${String(r.unknown).padStart(9)}\n`);
  }
...
if (a.format === 'json') process.stdout.write(JSON.stringify(out, null, 2) + '\n');
  ...
  for (const t of out) {
    ...
    process.stdout.write(`${t.cohort.padEnd(16)} ${t.kind.padEnd(19)} ${t.route}${pay}\n`);
  }
}
