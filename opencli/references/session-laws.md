...
**两种模式都不抢焦点。** 2026-08-21 九个并发 agent 跨三个 driver，
在每次导航前后都检查了最前端应用；每一次检查里宿主应用都保持在最前，
`--window foreground` 和 `--window background` 在这条轴上没有可观测差异。
如果有人报告屏幕「一直在跳」，成因是几个任务在写同一个共享页面——
那看起来像抖动，实际是法律 1 被违反了。

只有用户明确说要看着的时候才用前台。如果一个站点不抢焦点就没法操作，
停下来把这个约束报告出去。
