#!/usr/bin/env bash
# Semrush Site Audit · 问题清单（review/issues）双证人采集（见同目录 PAGE.md）
...
CID="${1:-31025602}"
OUT="${2:-$ROOT/backlink/evidence/ground-truth/semrush-siteaudit-issues-$(date +%Y%m%d-%H%M%S)}"

URL="https://sem.3ue.co/siteaudit/campaign/${CID}/review/issues"
